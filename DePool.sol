...
import "ConfigParamsBase.sol";
import "ParticipantBase.sol";
...
contract DePool is ValidatorBase, ProxyBase, ConfigParamsBase, ParticipantBase, RoundsBase, IDePool {
    ...
    // mapping of allowed participants
        ...
        public
    {
        ...
        tvm.accept();
        // set validator as a first allowed participant
        allowedParticipants[validatorWallet] = true;
        for (uint8 i = 0; i < 2; ++i) {
            address proxy =
    ...
    /// @dev set allowed participant
    function setAllowedParticipant(address _allowedParticipant) public onlyValidatorContract {
        require(allowedParticipants.keys().length < 2, Errors.MAX_PARTICIPANT_COUNT_REACHED);
        tvm.rawReserve(address(this).balance - msg.value, 0);
        allowedParticipants[_allowedParticipant] = true;
    ...
    function _returnOrReinvestForParticipant(
        ...
        // Add reportFee to the onRoundComplete callback
        uint64 reportFee = 0.01 ever;
        if (attachedValue < reportFee && newStake >= reportFee && !isValidator) {
            newStake -= reportFee;
            attachedValue += reportFee;

            // extract report fee from revard
            if (reward > reportFee) {
                reward -= reportFee;
            } else {
                reward = 0;
    ...
    function addOrdinaryStake(uint64 stake) public onlyInternalMessage {
        ...
        if (!allowedParticipants.exists(msg.sender)) {
            return _sendError(STATUS_NOT_ALLOWED_PARTICIPANT, 0);
        }
    ...
    function transferStake(address dest, uint64 amount) public onlyInternalMessage {
        ...
        IParticipant(dest).onTransfer{bounce: false, flag: 1}(src, transferred);
        sendAcceptAndReturnChange();
    }
